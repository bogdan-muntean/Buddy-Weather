...
function App() {
  ...
  // const [background, setBackground] = useState("b-default");

  useEffect(() => {
    const fetchWeather = async () => {
      await getFormattedWeatherData({ ...query, units: units }, units).then(
        (data) => {
          setWeather(data);
        }
      );
    ...
    <div className={`App`} id={`${background()}`}>
      ...
      {weather && (
        <>
          ...
          <Forecast
            forecastData={weather.hourlyForecast}
            title="hourly forecast"
          ></Forecast>
          <Forecast
            forecastData={weather.dailyForecast}
